...
aws iam get-policy --policy-arn arn:aws:iam::561243041928:policy/AllowExternalDNSUpdates
```
...
```sh
# Policy ARN
arn:aws:iam::561243041928:policy/AllowExternalDNSUpdates
```  
...
- [Get latest external dns image name](https://github.com/kubernetes-sigs/external-dns/releases/tag/v0.13.5)
```yaml
    spec:
      ...
      - name: external-dns
        ...
        # latest image: registry.k8s.io/external-dns/external-dns:v0.13.5
```
...
```sh
# Deploy external DNS
kubectl apply -f 05-External-DNS/01-deploy-external-dns.yaml
