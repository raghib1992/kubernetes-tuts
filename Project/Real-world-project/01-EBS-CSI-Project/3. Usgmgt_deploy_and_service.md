# Deploy UserManagement (application for demo) Service with MySQL Database
...
```sh
#create configmap for usermgmt deployemnt env
# kubectl create configmap usermgmt-config from-env-file=./usermgmt.properties

# Create Deployment for usermgmt
kubectl apply -f manifest-file/10-Usermgmt-Deployment.yaml

# Create Nodeport for usermgmt
kubectl apply -f manifest-file/11-UserManagement-Service.yml
...
### `Note: Check for security-group rule for http`
